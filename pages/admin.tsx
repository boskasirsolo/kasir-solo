...
import { Plus, Trash2, CheckCircle2, AlertCircle, Sparkles, UploadCloud, Loader2, Edit, ChevronLeft, ChevronRight, Save, X as XIcon, Tag, DollarSign } from 'lucide-react';
...
export const AdminDashboard = ({ 
  ...
  const [newProdDesc, setNewProdDesc] = useState(''); // Long Description
  const [prodShortDesc, setProdShortDesc] = useState(''); // Trigger AI
  ...
  const [galleryShortDesc, setGalleryShortDesc] = useState(''); // Trigger only
  const [galleryLongDesc, setGalleryLongDesc] = useState('');   // Main content
  ...
  const generateProductDescription = async () => {
    ...
    try {
      const prompt = `
        ...
        Instruksi:
        1. Fokus pada solusi dan manfaat (benefit) bagi pemilik usaha.
        2. Gaya bahasa profesional, meyakinkan, dan menjual.
        3. Jelaskan spesifikasi teknis dalam bahasa yang mudah dimengerti awam.
        4. Output: HANYA teks deskripsi (2-3 paragraf). Bahasa Indonesia.
      `;
  ...
  const handleProductSubmit = async () => {
    ...
    try {
      ...
      } else if (!editingProdId && !prodUploadFile) {
        // Optional: Force image on create
        // alert("Pilih gambar produk!"); return; 
      }
  ...
  const generateGalleryNarrative = async () => {
    ...
    try {
      const prompt = `
        ...
        Tugas: Buat narasi deskripsi panjang (storytelling) untuk portofolio/galeri website perusahaan Mesin Kasir.
        
        Data:
        - Judul Project: ${galleryTitle}
        - Konteks/Keyword Trigger: ${galleryShortDesc}
        
        Instruksi:
        1. Riset keyword turunan yang relevan dengan trigger secara otomatis dan masukkan ke dalam narasi secara natural.
        2. Gunakan konsep SEO modern (Semantic Search/LSI).
        3. Gaya bahasa: Profesional, solutif, meyakinkan, namun tetap mengalir seperti bercerita (storytelling).
        4. Struktur: Terdiri dari 2-3 paragraf pendek yang enak dibaca.
        5. Output: HANYA teks narasi saja. Jangan ada pembuka/penutup. Bahasa Indonesia baku.
      `;
      ...
      alert("Gagal generate Narasi AI"); 
      console.error(e);
    } 
      ...
      <div className="bg-brand-card border border-white/10 rounded-2xl p-8 min-h-[600px]">
              ...
              <div className="bg-brand-dark p-6 rounded-xl border border-white/5">
                ...
                <div className="p-4 bg-brand-card/50 rounded-lg border border-brand-orange/20 relative overflow-hidden mt-4">
                     <div className="absolute top-0 right-0 p-2 opacity-10">
                        <Sparkles size={100} className="text-brand-orange" />
                     </div>
                     <label className="text-xs text-brand-orange uppercase font-bold tracking-wider mb-2 block flex items-center gap-2">
                       <Sparkles size={14} /> AI Sales Copy Generator
                     </label>
                     <p className="text-xs text-gray-500 mb-2">Masukkan poin fitur utama, AI akan membuat deskripsi penjualan yang persuasif.</p>
                     <div className="flex gap-2">
                        <TextArea 
                          value={prodShortDesc} 
                          onChange={e => setProdShortDesc(e.target.value)} 
                          placeholder="Contoh: printer cepat, layar sentuh responsif, garansi 1 tahun, free training..." 
                          className="h-20 text-sm" 
                        />
                        <button 
                          onClick={generateProductDescription} 
                          disabled={isGeneratingProdAI}
                          className="w-32 bg-gradient-to-br from-purple-600 to-blue-600 text-white font-bold rounded-lg flex flex-col items-center justify-center gap-1 hover:brightness-110 transition-all shadow-lg"
                        >
                ...
                <div className="flex-grow overflow-y-auto p-4 space-y-3 custom-scrollbar">
                  {paginatedProducts.map(p => (
                    <div key={p.id} 
                      ...
                      className={`relative flex gap-3 p-3 rounded-lg border cursor-pointer transition-all group ${
                        editingProdId === p.id 
                          ? 'bg-brand-orange/10 border-brand-orange shadow-neon-text' 
                    ...
                    >
                      ...
                      <button 
                        ...
                        className="self-start text-gray-600 hover:text-red-500 transition-colors p-1"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
                ...
                {prodTotalPages > 1 && (
                  <div className="p-4 border-t border-white/10 bg-black/20 flex justify-between items-center">
                    <button 
                      ...
                      disabled={prodPage === 1}
                      className="p-2 rounded bg-white/5 hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition-colors text-white"
                    >
                      <ChevronLeft size={16} />
                    </button>
        ...
        {activeTab === 'gallery' && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            {/* LEFT COLUMN: EDITOR (70% -> col-span-8) */}
            <div className="lg:col-span-8 space-y-6">
              <div className="bg-brand-dark p-6 rounded-xl border border-white/5">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xl font-bold text-white flex items-center gap-2">
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 bg-brand-card/50 rounded-lg border border-brand-orange/20 relative overflow-hidden">
                     ...
                     <label className="text-xs text-brand-orange uppercase font-bold tracking-wider mb-2 block flex items-center gap-2">
                       <Sparkles size={14} /> AI Magic Trigger
                     </label>
                     <p className="text-xs text-gray-500 mb-2">Tulis keyword/poin singkat, AI akan membuatkan narasi SEO lengkap.</p>
                     <div className="flex gap-2">
                        <TextArea 
                          value={galleryShortDesc} 
                          onChange={e => setGalleryShortDesc(e.target.value)} 
                          placeholder="Contoh: kafe outdoor, wifi kencang, kasir cepat, antrian panjang jadi lancar..." 
                          className="h-20 text-sm" 
                        />
                        <button 
                  ...
                  <div>
                    ...
                    <TextArea 
                      value={galleryLongDesc} 
                      onChange={e => setGalleryLongDesc(e.target.value)} 
                      placeholder="Hasil generate AI akan muncul di sini. Bisa diedit manual." 
            ...
            {/* RIGHT COLUMN: LIST (30% -> col-span-4) */}
            <div className="lg:col-span-4 flex flex-col h-full">
              <div className="bg-brand-dark rounded-xl border border-white/5 flex flex-col h-full overflow-hidden">
                <div className="p-4 border-b border-white/10 bg-black/20">
