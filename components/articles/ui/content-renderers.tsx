    ...
    return (
        <div className="my-6 p-4 rounded-xl border border-blue-500/30 bg-blue-900/10 flex items-center gap-4 hover:bg-blue-900/20 transition-all group">
            ...
            <div className="flex-1 min-w-0">
                <h5 className="font-bold text-white text-xs truncate">{label || 'Download File'}</h5>
...
export const ProjectEmbedCard: React.FC<{ title: string, url: string, image: string, desc: string }> = ({ title, url, image, desc }) => {
    ...
    // Cegah 'undefined' muncul di UI
    const displayDesc = desc && desc !== 'undefined' ? desc : "Dokumentasi proyek implementasi sistem oleh tim Mesin Kasir Solo.";
        ...
        <div className="my-8 group relative rounded-xl overflow-hidden bg-brand-dark border border-white/5 hover:border-brand-orange/50 transition-all shadow-lg">
            <div className="flex flex-col md:flex-row min-h-[180px] md:min-h-[220px]">
                <div className="w-full md:w-1/3 relative h-48 md:h-auto overflow-hidden bg-black shrink-0 border-b md:border-b-0 md:border-r border-white/5">
                    <img 
                        src={optimizeImage(image || 'https://images.unsplash.com/photo-1556742049-0cfed4f7a07d', 500)} 
                        alt={title} 
                        className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-90 group-hover:opacity-100" 
                        loading="lazy" 
                    />
                    <div className="absolute top-2 left-2 flex gap-2">
                        <span className="bg-brand-orange text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-widest shadow-neon">
                            PROJECT CARD
                        </span>
                    </div>
                </div>

                <div className="p-4 md:p-6 flex flex-col justify-center w-full md:w-2/3 relative z-10">
                    ...
                    <div className="mt-auto pt-3 border-t border-white/5 flex items-center justify-between">
                        ...
                        <LinkComponent 
                            {...linkProps} 
                            className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-brand-orange text-white rounded-lg text-[10px] font-black border-none shadow-neon transition-all hover:bg-brand-action group/btn"
                        >
...
export const ProductEmbedCard = ({ name, price, image, desc, waNumber }: { name: string, price: string, image: string, desc: string, waNumber?: string }) => {
    const rawPrice = typeof price === 'string' ? parseInt(price.replace(/[^0-9]/g, '')) : price;
    const displayPrice = rawPrice > 0 ? formatRupiah(rawPrice) : (price || 'Cek Harga');
    
    const targetWa = waNumber || "6282325103336";
    const displayDesc = desc && desc !== 'undefined' ? desc : "Paket mesin kasir pilihan terbaik untuk efisiensi bisnis Anda.";
    ...
    return (
        <div className="my-8 bg-black/40 border border-white/5 rounded-xl overflow-hidden hover:border-brand-orange/50 transition-all shadow-lg flex flex-col md:flex-row group relative min-h-[160px]">
            <div className="w-full md:w-40 h-40 md:h-auto bg-black relative shrink-0 flex items-center justify-center p-3 border-b md:border-b-0 md:border-r border-white/5">
                <img 
                    src={optimizeImage(image || 'https://via.placeholder.com/300', 300)} 
                    alt={name} 
            ...
            <div className="p-4 md:p-6 flex flex-col justify-center flex-1">
                <h4 className="text-sm md:text-base font-bold text-white mb-1 leading-tight group-hover:text-brand-orange transition-colors">{name || 'Nama Produk'}</h4>
                <p className="text-xl font-display font-black text-brand-orange mb-3 tracking-tight">{displayPrice}</p>
                <p className="text-[10px] text-gray-500 leading-relaxed mb-4 line-clamp-2">{displayDesc}</p>
