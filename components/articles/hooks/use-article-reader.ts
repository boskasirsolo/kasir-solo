...
export const useArticleReader = (content: string) => {
  ...
  // Intersection Observer for Active Heading (H1-H10)
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    
    const timeout = setTimeout(() => {
      // UPDATED: Watch standard H tags AND our custom .deep-heading class
      const headings = container.querySelectorAll('h1, h2, h3, h4, h5, h6, .deep-heading'); 
