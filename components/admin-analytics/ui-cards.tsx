...
export const ReferrerList = ({ referrers, totalViews }: { referrers: [string, number][], totalViews: number }) => {
    ...
    return (
        <div className="bg-brand-dark border border-white/5 rounded-3xl p-6 md:p-7 flex flex-col h-full min-h-[465px] shadow-2xl relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start relative z-10 mb-6">
                <div>
                    <h4 className="text-white font-black text-sm flex items-center gap-2 uppercase tracking-widest">
                        <Globe size={18} className="text-blue-400"/> Pintu Masuk (Traffic)
                    </h4>
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-widest mt-1">Darimana calon juragan dateng:</p>
                </div>
                {totalPages > 1 && (
                    <div className="flex items-center gap-2">
                        <button 
                            ...
                            className="p-1 rounded-lg bg-white/5 hover:bg-white/10 disabled:opacity-20 transition-colors border border-white/10 text-white"
                        >
                            <ChevronLeft size={14}/>
                        </button>
            ...
            <div className="space-y-4 flex-1 relative z-10">
                ...
                {currentData.length > 0 && currentData.length < ITEMS_PER_PAGE && (
                    Array.from({ length: ITEMS_PER_PAGE - currentData.length }).map((_, i) => (
                        <div key={`empty-ref-${i}`} className="h-[46px] border border-white/[0.01] rounded-xl"></div>
                    ))
                )}
