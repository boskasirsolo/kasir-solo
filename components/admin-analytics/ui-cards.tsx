...
export const ReferrerList = ({ referrers, totalViews }: { referrers: [string, number][], totalViews: number }) => {
    ...
    return (
        <div className="bg-brand-dark border border-white/5 rounded-3xl p-6 md:p-8 flex flex-col h-full min-h-[460px] shadow-2xl relative overflow-hidden group">
            ...
            {/* Menggunakan gap yang lebih rapat (space-y-3.5) dan menghilangkan scrollbar */}
            <div className="space-y-3.5 flex-1 relative z-10">
                ...
                {/* Filler item disesuaikan tingginya agar kartu tidak 'melompat' saat ganti halaman */}
                {currentData.length > 0 && currentData.length < ITEMS_PER_PAGE && (
                    Array.from({ length: ITEMS_PER_PAGE - currentData.length }).map((_, i) => (
                        <div key={`empty-ref-${i}`} className="h-[34px] border border-white/[0.01] rounded-xl"></div>
                    ))
                )}
