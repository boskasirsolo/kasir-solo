...
export const DeviceStats = ({ devices, totalViews }: { devices: { mobile: number, desktop: number, tablet: number }, totalViews: number }) => (
    <div className="bg-brand-dark border border-white/5 rounded-xl p-6 flex flex-col h-full shadow-xl">
...
const DeviceBar = ({ label, count, total, icon: Icon, color }: any) => (
    <div>
        <div className="flex justify-between items-center text-[10px] font-bold uppercase mb-1.5">
            <span className="text-gray-500 flex items-center gap-2"><Icon size={12}/> {label}</span>
