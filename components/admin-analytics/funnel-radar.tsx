...
import { TrendingUp, AlertTriangle, ArrowDown, Target, Zap } from 'lucide-react';
...
export const FunnelRadar = ({ data }: { data: FunnelStats }) => {
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            {/* CORONG VISUAL (LEFT 7 COL) */}
            <div className="lg:col-span-7 bg-brand-dark border border-white/5 rounded-2xl p-6 shadow-xl relative overflow-hidden">
                ...
                <div className="mt-8 p-4 bg-brand-orange/5 border border-brand-orange/20 rounded-xl flex gap-4 items-center">
                    <div className="p-3 bg-brand-orange/10 rounded-full text-brand-orange animate-pulse">
                        <Zap size={20} />
                    </div>
                    <div>
                        <p className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Overall Success Rate</p>
                        <h3 className="text-2xl font-display font-black text-white">{data.conversionRate.toFixed(1)}%</h3>
                    </div>
                    <div className="ml-auto text-right">
                        <p className="text-[9px] text-gray-600 leading-relaxed italic max-w-[150px]">
                            *Persentase user yang akhirnya klik WA/Checkout.
                        </p>
                    </div>
                </div>
            </div>

            {/* GOLDEN PATHS (RIGHT 5 COL) */}
            <div className="lg:col-span-5 flex flex-col gap-6">
                <div className="bg-brand-dark border border-white/5 rounded-2xl p-6 shadow-xl flex-1">
                ...
                <div className="bg-red-950/20 border border-red-500/30 rounded-2xl p-6">
                    <h4 className="text-red-500 font-bold text-xs mb-3 flex items-center gap-2 uppercase tracking-wider">
                        <AlertTriangle size={14}/> Radar Boncos
                    </h4>
                    <p className="text-[10px] text-gray-400 leading-relaxed">
                        Tahap <strong>KEPO → NAKSIR</strong> punya drop-off paling tinggi (70%). 
                        Artinya banyak yang baca artikel tapi gak lirik produk.
                        <br/><br/>
                        <span className="text-white font-bold underline">Saran SIBOS:</span> Tambahin tombol 'Cek Harga' di tengah artikel biar user gak capek scroll.
                    </p>
                </div>
            </div>
