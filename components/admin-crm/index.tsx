...
import { Search, RefreshCw, ShoppingCart, Zap, List, X, LayoutDashboard, Sparkles, Ghost, Database, Cpu } from 'lucide-react';
...
export const AdminCRM = ({ config }: { config: any }) => {
    ...
    // URUTAN BARU: shadow -> simulations -> pipeline -> orders -> list
        ...
        <div className="space-y-6 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-brand-card/30 p-4 rounded-2xl border border-white/5 shadow-lg">
                <div className="flex bg-black/40 p-1 rounded-xl border border-white/10 w-full overflow-x-auto custom-scrollbar-hide">
                    {/* 1. SHADOW (Leads Pertama) */}
                    <button onClick={() => setActiveSubTab('shadow')} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'shadow' ? 'bg-brand-orange text-white shadow-neon' : 'text-gray-500'}`} title="Radar Abandoned Data">
                        <Ghost size={14}/> SHADOW
                    </button>
                    
                    {/* 2. SIMULASI (Leads Kedua) */}
                    <button onClick={() => setActiveSubTab('simulations')} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'simulations' ? 'bg-brand-orange text-white shadow-neon' : 'text-gray-500'}`} title="Blueprint Budget Juragan">
                        <Cpu size={14}/> SIMULASI
                    </button>

                    {/* 3. PIPELINE (Negosiasi Aktif) */}
                    <button onClick={() => setActiveSubTab('pipeline')} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'pipeline' ? 'bg-brand-orange text-white shadow-neon' : 'text-gray-500'}`} title="Kanban Strategy Board">
                        <LayoutDashboard size={14}/> PIPELINE
                    </button>

                    {/* 4. TRANSAKSI (Duit Masuk) */}
                    <button onClick={() => setActiveSubTab('orders')} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'orders' ? 'bg-brand-orange text-white shadow-neon' : 'text-gray-500'}`} title="Order Hardware Sah">
                        <ShoppingCart size={14}/> TRANSAKSI
                    </button>

                    {/* 5. DATABASE (Data Induk) */}
                    <button onClick={() => setActiveSubTab('list')} className={`flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'list' ? 'bg-brand-orange text-white shadow-neon' : 'text-gray-500'}`} title="Master Customer Data">
            ...
            {state.loading ? (
                ...
                <div className="animate-fade-in">
                    ...
                    {/* PANEL: TRANSAKSI (Hardware) */}
                    {activeSubTab === 'orders' && <OrdersPanel config={config} />}
                    
                    {/* PANEL: DATABASE (Data Lengkap) */}
                    {activeSubTab === 'list' && (
                        <CustomerTable 
                            customers={filteredCustomers} 
                            onOpenDetail={setSelectedCustomer} 
