    ...
    return (
        <div className="sticky top-0 z-30 bg-brand-black/80 backdrop-blur-md -mx-4 px-4 border-b border-white/5 shadow-xl py-4 flex flex-col gap-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
                ...
                {/* Search & Filter */}
                <div className="flex items-center gap-3 flex-1 md:max-w-2xl">
                    <div className="relative flex-1 group">
                        ...
                        <input 
                            ...
                            className="w-full bg-black/40 border border-white/10 rounded-2xl pl-12 pr-4 py-3 text-sm font-bold text-white outline-none focus:border-brand-orange focus:bg-black/60 transition-all h-12 placeholder:text-gray-700" 
                        />
                    </div>
                    <button 
                        onClick={() => setIsFilterOpen(!isFilterOpen)} 
                        className={`h-12 flex items-center gap-3 px-6 py-2 rounded-2xl border font-black text-xs uppercase tracking-widest transition-all relative active:scale-95 ${isFilterOpen || filterCount > 0 ? 'bg-brand-orange/10 border-brand-orange text-brand-orange shadow-neon-text/10' : 'bg-brand-card/50 border-white/5 text-gray-500 hover:text-white'}`}
                    >
                        <Filter size={16} />
