...
// FIX: Moved LeadCard definition above usage and added React.FC type to handle implicit key prop and resolve TS2322 error
            ...
            {radar && (
                <div className="absolute top-2 right-2 flex gap-1 opacity-40 group-hover:opacity-100 transition-opacity">
                    {customer.is_indecisive_buyer && (
                        <div className="bg-red-500/20 text-red-500 p-1 rounded border border-red-500/30 animate-pulse" title="LAGI GALAU">
                            <Zap size={10} />
                        </div>
                    )}
                </div>
...
export const PipelineModule = ({ customers, onOpenDetail, onStatusUpdate }: PipelineModuleProps) => {
    return (
