        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 md:p-8">
            ...
            <div className="relative w-full max-w-2xl bg-brand-dark border border-brand-orange/40 rounded-[2.5rem] shadow-[0_0_80px_rgba(0,0,0,0.9)] flex flex-col max-h-[90vh] overflow-hidden animate-fade-in">
                ...
                <div className="p-6 border-b border-white/10 bg-brand-card/50 flex justify-between items-center shrink-0 backdrop-blur-xl">
                    <div className="min-w-0 pr-4">
                        <div className="flex items-center gap-3 mb-2">
                            <span className={`text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-[0.2em] shadow-neon ${
                                customer.lead_temperature === 'hot' ? 'bg-red-600 text-white' : 'bg-brand-orange text-white'
                            }`}>
                                Surveillance Dossier
                            </span>
                        </div>
                        <h3 className="text-xl md:text-2xl font-black text-white truncate font-display tracking-tight">{customer.name}</h3>
                    </div>
                    <div className="flex gap-2 shrink-0">
                        <button 
                            onClick={handleWaDirect}
                            className="p-3 bg-green-500/10 hover:bg-green-500/20 text-green-500 rounded-2xl transition-all border border-green-500/20 active:scale-90"
                        >
                            <MessageCircle size={20} />
                        </button>
                        <button 
                            onClick={onClose} 
                            className="p-3 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-2xl transition-all border border-red-500/20 active:scale-90"
                        >
                            <X size={20} />
                        </button>
                    </div>
                ...
                <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar bg-black/20">
                    <div className="space-y-8">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <div className="bg-brand-card border border-white/5 p-4 rounded-2xl shadow-inner group hover:border-blue-500/30 transition-all">
                                <span className="text-gray-600 text-[10px] font-black uppercase tracking-widest block mb-2">Page Views</span>
                                <div className="flex items-center justify-between">
                                    <p className="text-2xl font-display font-black text-white group-hover:text-blue-400">{radar?.total_views || 0}</p>
                                    <Eye size={16} className="text-blue-500 opacity-40"/>
                                </div>
                            </div>
                            <div className="bg-brand-card border border-white/5 p-4 rounded-2xl shadow-inner group hover:border-purple-500/30 transition-all">
                                <span className="text-gray-600 text-[10px] font-black uppercase tracking-widest block mb-2">Mins Active</span>
                                <div className="flex items-center justify-between">
                                    <p className="text-2xl font-display font-black text-white group-hover:text-purple-400">{Math.round((radar?.avg_engagement_sec || 0)/60)}</p>
                                    <Clock size={16} className="text-purple-500 opacity-40"/>
                                </div>
                            </div>
                            <div className="bg-brand-card border border-white/5 p-4 rounded-2xl shadow-inner group hover:border-yellow-500/30 transition-all">
                                <span className="text-gray-600 text-[10px] font-black uppercase tracking-widest block mb-2">Confidence</span>
                                <div className="flex items-center justify-between">
                                    <p className="text-2xl font-display font-black text-white group-hover:text-yellow-500">{customer.lead_temperature === 'hot' ? '90%' : '45%'}</p>
                                    <Zap size={16} className="text-yellow-500 opacity-40"/>
                                </div>
                            </div>
                            <div className="bg-brand-card border border-white/5 p-4 rounded-2xl shadow-inner group hover:border-brand-orange/30 transition-all">
                                <span className="text-gray-600 text-[10px] font-black uppercase tracking-widest block mb-2">ID_MARK</span>
                                <div className="flex items-center justify-between">
                                    <p className="text-2xl font-display font-black text-white group-hover:text-brand-orange">{customer.phone.slice(-4)}</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {/* RADAR INTEL */}
                            <div className="bg-brand-card/40 border border-white/5 p-6 rounded-3xl">
                                <h4 className="text-xs font-black text-gray-500 mb-6 flex items-center gap-3 uppercase tracking-[0.2em]">
                                    <Activity size={16} className="text-brand-orange"/> Perilaku Digital
                                </h4>
                                <div className="space-y-5">
                                    <div>
                                        <label className="text-[10px] text-gray-600 font-black uppercase tracking-widest">Target Utama (Halaman)</label>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                                            <label className="text-[9px] text-gray-600 font-black uppercase block mb-1">Tipe Unit</label>
                                            <span className="text-xs text-blue-400 font-black uppercase">{radar?.top_category || 'General'}</span>
                                        </div>
                                        <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                                            <label className="text-[9px] text-gray-600 font-black uppercase block mb-1">Pintu Masuk</label>
                            ...
                            <div className="bg-brand-card/40 border border-white/5 p-6 rounded-3xl">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex justify-between items-start text-xs">
                                        ...
                                        <span className="text-gray-300 text-right font-medium italic pl-4">{intel.alamat || customer.location || '-'}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-brand-card/40 border border-white/5 p-6 rounded-3xl">
                            ...
                            <div className="space-y-5 max-h-[200px] overflow-y-auto custom-scrollbar pr-3">
                                {(customer.interaction_history || []).length > 0 ? customer.interaction_history.slice(0, 10).map((event: any, i: number) => (
                                    <div key={i} className="flex gap-4 items-start border-l-2 border-white/5 pl-5 relative group/event">
                                        ...
                                        <div className="flex-1">
                                            <p className="text-sm text-gray-300 leading-snug group-hover/event:text-white transition-colors">{event.event}</p>
                                            <p className="text-[10px] text-gray-600 mt-1.5 font-mono uppercase font-bold tracking-tighter">{new Date(event.date).toLocaleTimeString('id-ID')} • SYNC_OK</p>
                                        </div>
                                    </div>
                                )) : <p className="text-xs text-gray-700 italic text-center py-6 font-bold uppercase tracking-widest">Belum ada jejak terekam.</p>}
                            </div>
                        </div>
                ...
                <div className="p-6 bg-brand-card border-t border-white/10 flex flex-col gap-4 shrink-0 z-50 backdrop-blur-xl">
                    <div className="flex gap-3">
                        <div className="flex-1 space-y-1.5">
                            <label className="text-[9px] text-gray-600 font-black uppercase tracking-widest ml-2">Pipeline Status</label>
                            <select 
                                value={customer.lead_status}
                                onChange={(e) => updateStatus(customer.phone, e.target.value as LeadStatus)}
                                className="w-full bg-black/60 border border-white/10 text-xs font-black uppercase tracking-widest rounded-xl px-4 py-3 outline-none text-white appearance-none focus:border-brand-orange transition-all"
                            >
                                {PIPELINE_STAGES.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}
                            </select>
                        </div>
                        <div className="flex-1 space-y-1.5">
                            <label className="text-[9px] text-gray-600 font-black uppercase tracking-widest ml-2">Lead Temp</label>
                            <select 
                    ...
                    <div className="flex gap-3">
                        <button 
                            ...
                            className="flex-1 bg-brand-gradient text-white rounded-2xl py-4 font-black text-xs uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-neon active:scale-95 disabled:opacity-50 transition-all"
                        >
                        ...
                        <button 
                            ...
                            className="bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-600 hover:text-white rounded-2xl px-5 py-4 transition-all active:scale-90"
                        >
                            <Trash2 size={20} />
                        </button>
                    </div>
                ...
                <div className="p-3 bg-brand-dark text-center shrink-0 border-t border-white/[0.02]">
                    <p className="text-[7px] text-gray-800 font-black uppercase tracking-[0.5em]">Intellectual Property // PT MKS v3.2 PRO</p>
                </div>
            </div>
        </div>,
        document.body
