...
export const Timeline = ({ status, created_at }: { status: string, created_at: string }) => {
    ...
    return (
        <div className="relative my-8 px-4">
            ...
            <div className="absolute top-5 left-0 h-0.5 bg-brand-orange -z-0 transition-all duration-1000" style={{ width: `${progress}%` }}></div>

            <div className="flex justify-between relative z-10">
                <StatusStep active={status === 'pending'} completed={currentIndex > 0} icon={Clock} label="Nunggu Duit" date={new Date(created_at).toLocaleDateString('id-ID', {day: 'numeric', month: 'short'})} />
                <StatusStep active={status === 'paid'} completed={currentIndex > 1} icon={CheckCircle2} label="Udah Lunas" />
                <StatusStep active={status === 'processed'} completed={currentIndex > 2} icon={Package} label="Lagi Dirakit" />
                <StatusStep active={status === 'completed'} completed={status === 'completed'} icon={Truck} label="Meluncur" />
            </div>
        </div>
    );
};
