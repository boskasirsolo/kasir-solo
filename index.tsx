    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (prodData && prodData.length > 0) {
          ...
          // SEED PRODUCTS
          console.log("Database Produk kosong. Auto-Seeding...");
        ...
        // 2. ARTICLES FETCH (Bisa ditambah seeding juga kalau perlu)
        ...
        // 3. GALLERY FETCH & SEED (NEW LOGIC)
        const { data: galData } = await supabase.from('gallery').select('*').order('created_at', { ascending: false });
        if (galData && galData.length > 0) {
          setGallery(galData);
        } else {
          // SEED GALLERY
          console.log("Database Galeri kosong. Auto-Seeding...");
          // Kita hilangkan ID biar Supabase generate ID baru (sequence) atau paksa ID dari initial data
          // Karena kita pakai BIGINT GENERATED BY DEFAULT, kita bisa insert ID eksplisit
          const { data: seededGallery, error: seedError } = await supabase.from('gallery').insert(INITIAL_GALLERY).select();
          
          if (seededGallery && !seedError) {
             setGallery(seededGallery);
             console.log("Gallery seeded:", seededGallery.length, "items");
          } else {
